(function () {
  ...
  // Diagnostics verwijderd: debug-logger is een no-op (geen console-uitvoer).
  const dbg = function () {};
  ...
  (function surveyGuardModule() {
    ...
    // (#2 uitrol) Kill-switch/config voor de vragenlijst (chrome.storage.managed,
    // door IT via beleid te zetten). Fail-open: aan tenzij expliciet false.
    //   enabled              -> hoofdschakelaar
    //   surveyHelperEnabled  -> vragenlijst-checklist tonen
    //   surveyAuditEnabled   -> achtergrond-audit + Volgende-status-onderschepping
    const SURVEY_CONFIG = { enabled: true, surveyHelperEnabled: true, surveyAuditEnabled: true };
    // (#geen-vangnet) De leeftijdsafhankelijke verplichte vragen komen UITSLUITEND
    // uit het beheerscherm (config.surveys). Er is geen hard-coded terugval meer:
    // wat hier staat, is de enige bron van waarheid.
    let CONFIG_SURVEYS = [];
    // Handmatig in de UI-inspector aangewezen elementen voor de vragenlijst:
    //   surveyAgeSelector          -> waar de leeftijd van de cliënt staat
    //   surveyQuestionTextSelector -> het vraagtekst-element binnen elke vraag
    // Bron: config.uiSelectors (centraal) + storage.local onsHelper.uiOverrides.
    let UI_OVERRIDES = {};
    const ctLower = (s) => (s || '').replace(/\s+/g, ' ').trim().toLowerCase().replace(/\s*antwoord wissen\s*$/i, '').replace(/\s*\*+\s*$/, '').trim();
    function parsedSignal(value) {
      const source = value && typeof value === 'object' ? value : {};
      const text = String(source.text || '').trim();
      return {
        text,
        html: String(source.html || text),
        color: /^(?:#[0-9a-f]{6}|rgba?\(\s*\d+\s*,\s*\d+\s*,\s*\d+(?:\s*,\s*(?:0|1|0?\.\d+))?\s*\))$/i.test(source.color || '') ? source.color : '#704b00',
        bold: source.bold !== false,
        italic: !!source.italic,
        sizePx: Math.max(8, Math.min(40, parseInt(source.sizePx, 10) || 13))
      };
    }
    function parseSurveys(list) {
      const out = [];
      (Array.isArray(list) ? list : []).forEach((s) => {
        const nameLc = ctLower(s && s.name);
        const raw = Array.isArray(s && s.questions) ? s.questions
          : (s && s.question ? [{ question: s.question, op: s.op, value: s.value, value2: s.value2 }] : []); // compat: oude platte vorm
        const questions = [];
        raw.forEach((q, qIndex) => {
          const textLc = ctLower(q && q.question);
          if (!textLc || textLc.length < 4) return;
          const num = (x) => (x === '' || x == null || isNaN(parseInt(x, 10))) ? null : parseInt(x, 10);
          const legacyCondition = q && q.rule === 'leeftijd';
          const action = q && q.rule === 'signaal_tonen' ? 'signaal_tonen' : 'vraag_verplicht';
          questions.push({ surveyName: nameLc, textLc, rule: action, condition: (q && q.condition) || (legacyCondition ? 'leeftijd' : 'leeftijd'), signal: parsedSignal(q && q.signal), op: (q && q.op) || '>=', value: num(q && q.value), value2: num(q && q.value2), key: nameLc + '::' + textLc + '::' + qIndex });
        });
        if (nameLc && questions.length) out.push({ nameLc, questions });
      });
      return out;
    }
    function ageSatisfies(age, q) {
      if (!q) return false;
      if (q.condition === 'altijd') return true;
      if (age == null || q.condition !== 'leeftijd' || q.value == null) return false;
      const a = age, v = q.value, v2 = q.value2;
      switch (q.op) {
        case '=': return a === v;
        case '>': return a > v;
        case '<': return a < v;
        case '>=': return a >= v;
        case '<=': return a <= v;
        case '<>': return v2 != null && a >= v && a <= v2;   // binnen (grenzen inbegrepen)
        case '><': return v2 != null && (a < v || a > v2);   // buiten
        default: return false;
      }
    }
    function allConfigQuestions() { const out = []; CONFIG_SURVEYS.forEach((s) => { s.questions.forEach((q) => out.push(q)); }); return out; }
    function configQuestionsForText(text) {
      const L = ctLower(text);
      if (!L || L.length < 4) return [];
      // Match op de volledige vraagtekst (tolerant: begin/bevat, beide richtingen).
      return allConfigQuestions().filter((q) => q.textLc && (L === q.textLc || L.indexOf(q.textLc) !== -1 || q.textLc.indexOf(L) !== -1));
    }
    function configQuestionForText(text) { const all=configQuestionsForText(text);return all.find((q)=>q.rule==='vraag_verplicht')||all[0]||null; }
    function applySurveyConfig(cfg) {
      if (!cfg || typeof cfg !== 'object') return;
      const hc = (cfg.helperConfig && typeof cfg.helperConfig === 'object') ? cfg.helperConfig : cfg;
      const features = hc.features || cfg.features || {};
      if (typeof features.surveyRequired === 'boolean') SURVEY_CONFIG.surveyHelperEnabled = features.surveyRequired;
      if (typeof features.surveyAudit === 'boolean') SURVEY_CONFIG.surveyAuditEnabled = features.surveyAudit;
      if (typeof cfg.enabled === 'boolean') SURVEY_CONFIG.enabled = cfg.enabled;
      if (typeof cfg.surveyHelperEnabled === 'boolean') SURVEY_CONFIG.surveyHelperEnabled = cfg.surveyHelperEnabled;
      if (typeof cfg.surveyAuditEnabled === 'boolean') SURVEY_CONFIG.surveyAuditEnabled = cfg.surveyAuditEnabled;
      if (Array.isArray(hc.surveys)) CONFIG_SURVEYS = parseSurveys(hc.surveys);
      else if (Array.isArray(cfg.surveys)) CONFIG_SURVEYS = parseSurveys(cfg.surveys);
      // Centraal aangewezen UI-selectors als basis (lokaal herstel wint later).
      const us = (hc.uiSelectors && typeof hc.uiSelectors === 'object') ? hc.uiSelectors : null;
      if (us) { for (const k of ['surveyAgeSelector', 'surveyQuestionTextSelector', 'surveyClientNameSelector', 'surveyTitleSelector', 'surveyQuestionSelector', 'surveyNextStatusSelector']) if (typeof us[k] === 'string' && us[k]) UI_OVERRIDES[k] = us[k]; }
      try { scheduleRefresh(); } catch (e) {}
    }
    function loadSurveyConfig() {
      try {
        if (chrome && chrome.runtime && chrome.runtime.getURL) fetch(chrome.runtime.getURL('config.json')).then((r) => r.ok ? r.json() : null).then((cfg) => { if (cfg) applySurveyConfig(cfg); }).catch(() => {});
        if (chrome && chrome.storage && chrome.storage.managed) {
          chrome.storage.managed.get(null, (cfg) => { try { if (chrome.runtime && chrome.runtime.lastError) return; applySurveyConfig(cfg); } catch (e) {} });
        }
        if (chrome && chrome.storage && chrome.storage.local) {
          chrome.storage.local.get(['onsHelperConfig'], (res) => { try { if (res && res.onsHelperConfig) applySurveyConfig(res.onsHelperConfig); } catch (e) {} });
          // Lokaal in de UI-inspector aangewezen selectors (winnen per sleutel).
          chrome.storage.local.get(['onsHelper.uiOverrides'], (res) => { try { const o = res && res['onsHelper.uiOverrides']; if (o && typeof o === 'object') for (const k of ['surveyAgeSelector', 'surveyQuestionTextSelector', 'surveyClientNameSelector', 'surveyTitleSelector', 'surveyQuestionSelector', 'surveyNextStatusSelector']) if (typeof o[k] === 'string' && o[k]) UI_OVERRIDES[k] = o[k]; try { scheduleRefresh(); } catch (e) {} } catch (e) {} });
        }
      } catch (e) {}
    }
    function surveyDisabled() { return !SURVEY_CONFIG.enabled || !SURVEY_CONFIG.surveyHelperEnabled; }
    try { if (chrome && chrome.storage && chrome.storage.onChanged) chrome.storage.onChanged.addListener((c, area) => { if (area === 'managed' || area === 'local') loadSurveyConfig(); }); } catch (e) {}
    loadSurveyConfig();
    ...
    const _NEXT_STATUS_DEFAULT = 'button[title="Volgende status"],button[aria-label="Volgende status"]';
    const nextStatusSel = () => UI_OVERRIDES.surveyNextStatusSelector || _NEXT_STATUS_DEFAULT;
    const ct = (s) => (s || '').replace(/\s+/g, ' ').trim().toLowerCase();
    // (#geen-vangnet) Eén config-gedreven regel: een vraag op het scherm wordt
    // gematcht op de in het beheerscherm ingestelde vraagtekst en is verplicht
    // zodra de cliëntleeftijd aan de ingestelde vergelijking voldoet. Er is geen
    // hard-coded JG Outcome-regel meer; alles komt uit config.surveys.
    const VIRTUAL_RULE = {
      questionsFor: (text) => configQuestionsForText(text),
      questionFor: (text) => configQuestionForText(text),
      appliesAt: (cq, age) => ageSatisfies(age, cq),
      // Is er binnen dezelfde vragenlijst nóg een geconfigureerde vraag die bij
      // deze leeftijd wél hoort? (voor de "verkeerde leeftijdsversie"-waarschuwing)
      hasApplicableSibling: (age, cq) => allConfigQuestions().some((q) => q !== cq && q.rule === 'vraag_verplicht' && q.surveyName === (cq && cq.surveyName) && ageSatisfies(age, q)),
      // Legacy-shims (voor het geval iets nog naar de oude interface verwijst):
      groupOf: (text) => { const cq = configQuestionForText(text); return cq ? cq.key : null; },
      expectedGroupForAge: () => null,
      ageThreshold: 0,
      warningFor: () => ''
    };
    function surveyTitleOf(root) {
      ...
      const visibleTitle = doc.querySelector(UI_OVERRIDES.surveyTitleSelector || '.mod-survey-result-header h1.ivy-title,[data-react-component="SurveyResultHeader"] h1.ivy-title');
      return visibleTitle && visibleTitle.textContent ? visibleTitle.textContent.replace(/\s+/g, ' ').trim() : '';
    }
    // Vindt de geconfigureerde vragenlijst die bij de huidige titel hoort. Werkt
    // ook op het overzicht- en statusscherm, waar (nog) geen vraagblokken staan —
    // net zoals de oude JG Outcome-route op de titel matchte.
    function configSurveyByTitle(root) {
      if (!CONFIG_SURVEYS.length) return null;
      const t = ctLower(surveyTitleOf(root));
      if (!t) return null;
      return CONFIG_SURVEYS.find((s) => s.nameLc && (t === s.nameLc || t.indexOf(s.nameLc) !== -1 || s.nameLc.indexOf(t) !== -1)) || null;
    }
    // De config-regel is actief zodra (1) de vragenlijst-titel een geconfigureerde
    // vragenlijst is — zodat overzicht én "Volgende status" dezelfde route volgen
    // als bij Outcome — of (2) er minstens één geconfigureerde vraag op de pagina staat.
    function respondentRuleFor(root) {
      if (!CONFIG_SURVEYS.length) return null;
      if (configSurveyByTitle(root)) return VIRTUAL_RULE;
      let anyMatch = false;
      try { anyMatch = surveyQuestionBlocks(root || document).some((q) => configQuestionsForText(questionLabelText(q)).length); } catch (e) { anyMatch = false; }
      return anyMatch ? VIRTUAL_RULE : null;
    }
    // Behoudt de bestaande betekenis: "heeft deze vragenlijst leeftijdsregels?"
    // Alle callers blijven werken.
    function isJgOutcomeSurvey(root) {
      return !!respondentRuleFor(root);
    }
    ...
    // (#3) Wanneer de controle "Volgende status" blokkeert, onthouden we de knop
    // zodat de checklist een expliciete "Toch doorgaan"-noodknop kan tonen.
    let _pendingOverrideBtn = null;
    ...
    const _SURVEY_QUESTION_DEFAULT = '.survey_question,tr.required';
    const surveyQuestionSel = () => UI_OVERRIDES.surveyQuestionSelector || _SURVEY_QUESTION_DEFAULT;
    ...
    function isSurveyQuestionBlock(el) {
      return !!(el && el.matches && el.matches(surveyQuestionSel()));
    }
    function closestSurveyQuestionBlock(el) {
      return el && el.closest ? el.closest(surveyQuestionSel()) : null;
    }
    function surveyQuestionBlocks(root) {
      return Array.prototype.slice.call((root || document).querySelectorAll(surveyQuestionSel()))
    ...
    function questionLabelRaw(q) {
      // Handmatig aangewezen vraagtekst-element (geldt voor ELKE vraag) heeft
      // voorrang; anders de standaard .question-text.
      let lbl = null;
      if (UI_OVERRIDES.surveyQuestionTextSelector) { try { lbl = q.querySelector(UI_OVERRIDES.surveyQuestionTextSelector); } catch (e) {} }
      if (!lbl) lbl = q.querySelector('.question-text');
      if (!lbl) return '';
    ...
    function questionGroup(text, rule) {
      const r = rule || VIRTUAL_RULE;
      try { return r.groupOf(text); } catch (e) { return null; }
    }
    function respondentWarningText(age, wrongGroups, rule) {
      const groups = Array.isArray(wrongGroups) ? wrongGroups : [];
      if (age == null || !groups.length) return '';
      // Config-gedreven: er staat een antwoord ingevuld dat bij deze leeftijd niet
      // hoort, terwijl er wél een andere (leeftijds)versie van de vraag geldt.
      return 'Let op: er is een antwoord ingevuld dat bij deze leeftijd niet nodig is. Controleer of je de juiste leeftijdsversie van de vraag invult.';
    }
    ...
    function isRequiredQuestion(q, age, rule) {
      if (!q) return false;
      // (#geen-vangnet) Staat deze vraag in het beheerscherm? Dan bepaalt de config
      // ALLES: verplicht zodra de leeftijd aan de ingestelde vergelijking voldoet,
      // en anders NIET verplicht (ook al zou ONS de andere leeftijdsversie markeren).
      if (rule) {
        const configured = rule.questionsFor ? rule.questionsFor(questionLabelText(q)) : [rule.questionFor(questionLabelText(q))].filter(Boolean);
        const requiredRules=configured.filter((cq)=>cq.rule==='vraag_verplicht');
        if (requiredRules.length) return requiredRules.some((cq)=>rule.appliesAt(cq, age));
      }
      // Niet in de config: de gewone ONS-markering (sterretje/required) blijft gelden.
    ...
    // (#2) Bevat de vraag een besturingselement dat isAnswered betrouwbaar kan
    // beoordelen? Zo niet, dan mag een verplichte vraag niet vals-groen worden;
    // we markeren haar als "niet te controleren" zodat jij haar zelf nakijkt.
    function hasRecognizedControl(q) {
      if (!q || !q.querySelector) return false;
      return !!q.querySelector(
        'input[type="radio"]:not([disabled]),input[type="checkbox"]:not([disabled]),' +
        'select:not([disabled]),textarea:not([disabled]),' +
        'input:not([disabled]):not([type="hidden"]):not([type="button"]):not([type="submit"]):not([type="reset"]):not([type="image"]),' +
        '[contenteditable="true"],[data-react-component][data-props],' +
        'uc-select,uc-input,uc-textarea,[role="combobox"],[role="slider"],[aria-selected],[aria-checked]'
      );
    }
    function clientAgeYears() {
      const selectors = [
        // Handmatig in de UI-inspector aangewezen leeftijd-element: eerst proberen.
        ...(UI_OVERRIDES.surveyAgeSelector ? [UI_OVERRIDES.surveyAgeSelector] : []),
    ...
    function surveyClientName() {
      const docs=[],seen=new Set();const add=(doc)=>{if(doc&&!seen.has(doc)){seen.add(doc);docs.push(doc);}};
      // Handmatig aangewezen naam-element (UI-inspector) heeft voorrang.
      // Shadow-DOM-bewust: de cliëntheader zit in geneste shadow roots.
      if (UI_OVERRIDES.surveyClientNameSelector) {
        try {
          const hits = deepQueryAll(UI_OVERRIDES.surveyClientNameSelector) || [];
          for (const el of hits) { const n = String(el.textContent||'').replace(/\s+/g,' ').trim().replace(/\s*\(\d{1,3}\s*jaar\).*$/i,''); if (n) return n; }
        } catch (e) {}
      }
      try{add(window.top&&window.top.document);}catch(e){}add(document);
      try{Array.from((window.top&&window.top.document||document).querySelectorAll('iframe')).forEach((frame)=>{try{add(frame.contentDocument);}catch(e){}});}catch(e){}
      const clean=(raw)=>String(raw||'').replace(/\s+/g,' ').trim().replace(/\s*\(\d{1,3}\s*jaar\).*$/i,'');
      // Vormfields e.d. ("survey_result[question_1240]") zijn géén naam.
      const looksLikeName=(n)=>!!n && n.length<=100 && !/[\[\]{}]|survey_result|question_\d|_id\b/i.test(n) && /[a-zà-ÿ]/i.test(n);
      // De cliëntheader zit in (geneste) shadow roots; daarom shadow-bewust zoeken.
      const nameSelectors=['uc-person-name','.base-card__name-tags','[data-testid="client-details-name"]','[data-testid="client-name"]','[data-client-name]','[data-invitee-name]','[data-testid="avatar-modal-header"] [class*="name" i]','.client-name'];
      for(const sel of nameSelectors){let hits=[];try{hits=deepQueryAll(sel);}catch(e){}for(const el of hits){const raw=(el.getAttribute&&(el.getAttribute('data-client-name')||el.getAttribute('data-invitee-name')))||el.textContent||'';const name=clean(raw);if(looksLikeName(name))return name;}}
      // Laatste redmiddel: react-props met een expliciete cliëntnaam (géén
      // generieke props.name — dat pikt vormveldnamen op).
      for(const doc of docs){for(const host of Array.from(doc.querySelectorAll('[data-react-component][data-props]'))){try{const props=JSON.parse(host.getAttribute('data-props')||'{}'),name=props.clientName||(props.client&&props.client.name);if(looksLikeName(clean(name)))return clean(name);}catch(e){}}}
      return '';
    }
    ...
    // (#9) Alle van elkaar verschillende leeftijden die op de pagina staan. Meer
    // dan één betekent: meerdere cliënten in beeld -> automatische keuze is
    // onbetrouwbaar, dus laten we de gebruiker de respondent-leeftijd bevestigen.
    function distinctClientAges() {
      const ages = new Set();
      const docs = [];
      ...
      for (const doc of docs) {
        let text = '';
        try { text = (doc.body && doc.body.textContent) || ''; } catch (e) {}
        const re = /\((\d{1,3})\s*jaar\)/g;
        let m;
        while ((m = re.exec(text))) { const a = parseInt(m[1], 10); if (a >= 0 && a <= 125) ages.add(a); }
      }
      return Array.from(ages);
    }
    function clientAgeAmbiguous() { return distinctClientAges().length > 1; }
    // (#1) Handmatige leeftijd-terugval, per vragenlijst en kortlevend.
    const MANUAL_AGE_KEY = 'onsHelper.surveyManualAge';
    function loadManualAge() {
      try {
        const s = JSON.parse(sessionStorage.getItem(MANUAL_AGE_KEY) || 'null');
        if (!s || s.schema !== CHECKLIST_SCHEMA || !s.ts || Date.now() - s.ts > 4 * 60 * 60 * 1000) return null;
        const cur = currentSurveyResultId();
        if (s.resultId && cur && s.resultId !== cur) return null;
        return s;
      } catch (e) { return null; }
    }
    function saveManualAge(under12) {
      try {
        sessionStorage.setItem(MANUAL_AGE_KEY, JSON.stringify({
          schema: CHECKLIST_SCHEMA, resultId: currentSurveyResultId() || null,
          age: under12 ? 6 : 15, under12: !!under12, ts: Date.now()
        }));
      } catch (e) {}
    }
    function clearManualAge() { try { sessionStorage.removeItem(MANUAL_AGE_KEY); } catch (e) {} }
    // Effectieve leeftijd voor de controle: een handmatige keuze wint; anders de
    // gedetecteerde leeftijd, mits die eenduidig is. Onbekend of ambigu -> null,
    // zodat de aanroeper de gebruiker om bevestiging kan vragen.
    function resolvedAge() {
      const manual = loadManualAge();
      if (manual) return manual.age;
      const detected = clientAgeYears();
      if (detected != null && !clientAgeAmbiguous()) return detected;
      return null;
    }
    function evaluate(root, ageOverride, sourceUrl) {
      ...
      const rule = respondentRuleFor(doc);
      const catOrder = categoryOrderOf(doc);
      let qIndex = -1;
      for (const q of surveyQuestionBlocks(doc)) {
        qIndex++;
        const configuredQuestions = rule ? (rule.questionsFor ? rule.questionsFor(questionLabelText(q)) : [rule.questionFor(questionLabelText(q))].filter(Boolean)) : [];
        // Een opgeslagen antwoord uit de verkeerde leeftijdsversie kan verborgen
        // zijn. Detecteer het daarom vóór de zichtbaarheidsfilter: een geconfigureerde
        // vraag die is ingevuld maar bij deze leeftijd NIET hoort, terwijl er binnen
        // dezelfde vragenlijst wél een passende (andere) leeftijdsversie is.
        if (rule && age != null) {
          configuredQuestions.filter((cq)=>cq.rule==='vraag_verplicht').forEach((cq)=>{if(!rule.appliesAt(cq, age)&&isAnswered(q)&&rule.hasApplicableSibling(age,cq))wrongRespondentGroups.add(cq.key);});
        }
        configuredQuestions.filter((cq)=>cq.rule==='signaal_tonen'&&rule.appliesAt(cq,age)&&!questionInactiveByPageCode(q)&&cq.signal&&cq.signal.text).forEach((cq)=>signals.push({question:readableQuestionText(q),signal:cq.signal,key:cq.key}));
        if (questionInactiveByPageCode(q) || !isRequiredQuestion(q, age, rule)) continue;
        // (#2) Onbekend besturingselement -> niet als ingevuld tellen.
        const recognized = hasRecognizedControl(q);
        const unknown = !recognized;
        const done = recognized ? isAnswered(q) : false;
      ...
      const respondentWarning = respondentWarningText(age, wrongGroups, rule);
      return { age, items, missing, signals, wrongRespondentGroups: wrongGroups, respondentWarning, blocked: missing.length > 0 || !!respondentWarning };
    }
    // (Diagnose-hook window.__onsSurveyDiag verwijderd: geen uitleesbare
    // vraagteksten/leeftijd meer beschikbaar buiten de normale werking.)
    ...
    // Herbruikbare actieknop in het paneel (leeftijdkeuze, 'toch doorgaan').
    function surveyActionButton(label, kind, onClick) {
    ...
    // (#1/#9) Vraag de respondent-leeftijd wanneer die niet eenduidig is bepaald.
    // Zolang deze vlag aan staat, mag de periodieke refresh de prompt niet
    // overschrijven met de checklist.
    let _awaitingAgeChoice = false;
    function showAgePrompt(reason, onPick) {
      ...
      p.textContent = reason || 'De leeftijd kon niet automatisch worden bepaald. Is de cliënt jonger dan 12 jaar?';
      Object.assign(p.style, { margin: '2px 0 10px', lineHeight: '1.4' });
      msg.appendChild(p);
      const row = document.createElement('div');
      Object.assign(row.style, { display: 'flex', flexWrap: 'wrap', gap: '8px' });
      row.appendChild(surveyActionButton('Ja, jonger dan 12', 'primary', () => { saveManualAge(true); pick(6); }));
      row.appendChild(surveyActionButton('Nee, 12 jaar of ouder', 'warn', () => { saveManualAge(false); pick(15); }));
      msg.appendChild(row);
      b.style.display = 'flex';
    }
    // (#7) Aantal openstaande verplichte vragen als badge op het extensie-icoon.
    ...
    function sendBadge(count) {
      ...
      if (c === _lastBadge) return;
      _lastBadge = c;
      try { chrome.runtime.sendMessage({ type: 'badge', count: c }); } catch (e) {}
    }
    // (#4) Toon de voortgang van de achtergrond-audit in de paneelkop.
    function setAuditProgress(done, total) {
      if (!bannerEl || bannerEl.style.display === 'none') return;
      const title = bannerEl.querySelector('[data-survey-title]');
      if (title && total > 1) title.textContent = 'Categorie ' + done + ' van ' + total + ' controleren…';
    }
    ...
    function managedSignalNode(value, tagValues) {
      const signal = parsedSignal(value);
      const out = document.createElement('div');
      const parsed = new DOMParser().parseFromString('<body>' + signal.html + '</body>', 'text/html');
      const allowed = new Set(['B','STRONG','I','EM','U','S','STRIKE','A','MARK','FONT','BR','P','DIV','SPAN','H2','H3','H4','BLOCKQUOTE','PRE','CODE','UL','OL','LI']);
      Array.prototype.slice.call(parsed.body.querySelectorAll('script,style,iframe,object,embed,img,svg,form,input,button')).forEach((el) => el.remove());
      Array.prototype.slice.call(parsed.body.querySelectorAll('*')).forEach((el) => {
        if (!allowed.has(el.tagName)) { el.replaceWith.apply(el, Array.prototype.slice.call(el.childNodes)); return; }
        const href = el.tagName === 'A' ? String(el.getAttribute('href') || '') : '';
        const styles = { color:el.style.color, backgroundColor:el.style.backgroundColor, fontWeight:el.style.fontWeight, fontStyle:el.style.fontStyle, textDecoration:el.style.textDecoration };
        Array.prototype.slice.call(el.attributes).forEach((attr) => el.removeAttribute(attr.name));
        if (el.tagName === 'A' && /^(?:https?:\/\/|mailto:|#)/i.test(href)) { el.setAttribute('href', href); el.setAttribute('target', '_blank'); el.setAttribute('rel', 'noopener noreferrer'); }
        Object.keys(styles).forEach((prop) => { if (styles[prop]) { try { el.style[prop] = styles[prop]; } catch (e) {} } });
      });
      out.innerHTML = parsed.body.innerHTML || signal.text;
      const tags=tagValues||{},replace=(text)=>String(text||'').replace(/\{vraag\}/gi,tags.vraag?String(tags.vraag):'{vraag}').replace(/\{clientNaam\}/gi,tags.clientNaam||'{clientNaam}').replace(/\{clientLeeftijd\}/gi,tags.clientLeeftijd==null?'{clientLeeftijd}':String(tags.clientLeeftijd));
      const walk=(node)=>{if(node.nodeType===3){node.nodeValue=replace(node.nodeValue);return;}Array.from(node.childNodes||[]).forEach(walk);};walk(out);
      Object.assign(out.style, { color:signal.color, fontWeight:signal.bold ? '700' : '400', fontStyle:signal.italic ? 'italic' : 'normal', fontSize:signal.sizePx + 'px', overflowWrap:'anywhere' });
      return out;
    }
    function renderChecklist(state) {
      ...
      const unknownRemaining = state.items.filter((item) => item.unknown && !item.done).length;
      ...
      if (titleEl && !remaining && hasSignals) titleEl.textContent = signals.length + ' ' + (signals.length === 1 ? 'signaal' : 'signalen');
      if (collapse) {
      ...
      setDock(true);
      // Noodknop om de "Volgende status"-blokkade te negeren. Alleen zichtbaar
      // zolang er een geblokkeerde statusknop wacht en er nog vragen open staan.
      (function addOverrideToolbar() {
        if (!_pendingOverrideBtn || !remaining) return;
        const bar = document.createElement('div');
        Object.assign(bar.style, { display: 'flex', gap: '8px', margin: '2px 0 8px', flexWrap: 'wrap' });
        const btn = _pendingOverrideBtn;
        bar.appendChild(surveyActionButton('Toch doorgaan', 'warn', () => {
          _pendingOverrideBtn = null;
          try { statusBypass.add(btn); btn.click(); } finally { setTimeout(() => statusBypass.delete(btn), 0); }
          hideBanner();
        }));
        msg.appendChild(bar);
      })();
      signals.forEach((entry) => {
        ...
        signalBox.setAttribute('role', 'alert');
        Object.assign(signalBox.style, { margin:'4px 0 10px', padding:'10px 11px', borderRadius:'7px', border:'1px solid #f0b429', background:'#fff7d6', color:'#704b00', lineHeight:'1.4' });
        const label = document.createElement('div'); label.textContent = entry.question || 'Signaal';
        Object.assign(label.style, { marginBottom:'4px', fontSize:'11px', fontWeight:'800', textTransform:'uppercase', letterSpacing:'.02em', color:'#704b00' });
        signalBox.append(label, managedSignalNode(entry.signal,{vraag:entry.question||'',clientNaam:surveyClientName(),clientLeeftijd:state.age==null?resolvedAge():state.age}));
        msg.appendChild(signalBox);
      });
      ...
      for (const entry of grouped.values()) {
        ...
        for (const item of entry.items) {
          ...
          const isUnknown = !!item.unknown && !item.done;
          link.textContent = item.question + (isUnknown ? '  — niet automatisch te controleren' : '');
          if (isUnknown) link.title = 'Deze verplichte vraag kon niet automatisch worden gecontroleerd. Kijk haar zelf na.';
          const openColor = isUnknown ? '#b45309' : '#cc087d';
          Object.assign(link.style, {
            flex: '1 1 auto', minWidth: '0',
            color: item.done ? '#9aa0a6' : openColor,
          ...
          // (#8) Elk item is via het toetsenbord te bereiken en te openen.
          link.tabIndex = 0;
          link.addEventListener('keydown', (e) => {
            if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); link.click(); return; }
            if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
              e.preventDefault();
              const links = Array.prototype.slice.call(msg.querySelectorAll('a[tabindex="0"]'));
          ...
          mark.textContent = item.done ? '✓' : (isUnknown ? '?' : '');
          mark.setAttribute('aria-label', item.done ? 'Ingevuld' : (isUnknown ? 'Niet te controleren' : 'Nog niet ingevuld'));
          Object.assign(mark.style, (item.done || isUnknown) ? {
            flex: '0 0 auto', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', marginTop: '1px',
            width: '18px', height: '18px', borderRadius: '999px',
            background: item.done ? '#1b7f3b' : '#b45309', color: '#fff', fontWeight: '900', fontSize: '11px', lineHeight: '1'
          } : { display: 'none' });
    ...
    function showMissingBanner(res, replaceExisting) {
      ...
      const incoming = (res.items || res.missing || []).map((item) => {
        ...
        return {
          ...
          unknown: !!item.unknown && !(local && hasRecognizedControl(local)),
          done: !!item.done || !!previousDone.get(key) || !!(local && hasRecognizedControl(local) && isAnswered(local))
        };
      });
      ...
      for (const item of incoming) {
        ...
        if (!known.has(key)) { known.add(key); items.push(item); }
          ...
          if (existing) existing.unknown = item.unknown;
        }
      }
      ...
      const state = {
        ...
        ts: Date.now(),
        age: res.age == null ? (previous && previous.age) : res.age,
    ...
    function updateChecklistCompletion() {
      ...
      for (const item of state.items) {
        ...
        const recognized = hasRecognizedControl(question);
        const done = recognized ? isAnswered(question) : false;
        const unknown = !recognized;
        if (item.done !== done) { item.done = done; changed = true; }
        if (item.unknown !== unknown) { item.unknown = unknown; changed = true; }
      }
      state.items = kept;
    ...
    function hideBanner() { setDock(false); if (bannerEl) bannerEl.style.display = 'none'; sendBadge(0); }
    function refresh() {
      // Leeftijd-prompt actief? Laat die staan tot de gebruiker gekozen heeft.
      if (_awaitingAgeChoice) return;
      ...
      if (res.signals && res.signals.length) { showMissingBanner(res, true); return; }
      hideBanner();
    }
    ...
    async function evaluateAllSurveyCategories(ageOverride, onProgress) {
      const runAll = (list, ag) => {
        let done = 0;
        const total = list.length;
        if (onProgress) { try { onProgress(0, total); } catch (e) {} }
        return Promise.all(list.map((url) => loadRenderedSurveyMissing(url, ag).then((r) => {
          done++;
    ...
    function prepareStatusAudit() {
      ...
      // (#1/#9) Gebruik de effectieve leeftijd (handmatige keuze wint; anders
      // eenduidig gedetecteerd), zodat een handmatige bevestiging doorwerkt.
      const detectedAge = resolvedAge();
      ...
      statusAuditPromise = evaluateAllSurveyCategories(detectedAge, setAuditProgress).catch((err) => {
    ...
    async function onNextStatusClick(e) {
      ...
      const btn = t.closest(nextStatusSel());
      ...
      // (#2 uitrol) Audit/onderschepping centraal uit? Laat ONS de knop normaal
      // afhandelen; niet onderscheppen.
      if (!SURVEY_CONFIG.enabled || !SURVEY_CONFIG.surveyAuditEnabled) return;
      e.preventDefault();
      e.stopPropagation();
      ...
      try {
        ...
        // (#1/#9) Geen eenduidige leeftijd (onbekend of meerdere cliënten)?
        // Vraag het de gebruiker en herstart de controle met die keuze.
        if (resolvedAge() == null) {
          const reason = clientAgeAmbiguous()
            ? 'Er zijn meerdere cliënten in beeld. Geldt deze vragenlijst voor een cliënt jonger dan 12 jaar?'
            : 'De leeftijd van de cliënt kon niet automatisch worden bepaald. Is de cliënt jonger dan 12 jaar?';
          showAgePrompt(reason, () => {
            statusAuditPromise = null; statusAuditPage = ''; statusAuditAge = undefined;
            onNextStatusClick(e);
          });
          return;
        }
        ...
        if (res.age == null) {
          clearAuditResume();
          showAgePrompt('De leeftijd kon niet worden vastgesteld. Is de cliënt jonger dan 12 jaar?', () => {
            statusAuditPromise = null; statusAuditPage = ''; statusAuditAge = undefined;
            onNextStatusClick(e);
          });
          return;
        ...
        if (res.blocked) {
          _pendingOverrideBtn = btn; // (#3) noodknop "Toch doorgaan" tonen
    ...
    function tick() {
      // (#2 uitrol) Centraal uitgeschakeld? Niets tonen/doen.
      if (surveyDisabled()) { hideBanner(); return; }
      ...
      if (isSurveyPage()) {
        ...
        refresh();
      } else if (document.querySelector(nextStatusSel()) || surveyEditUrlFromOverview()) {
        // Start voordat de gebruiker klikt; meestal is het resultaat dan direct beschikbaar.
        if (SURVEY_CONFIG.surveyAuditEnabled && (!isJgOutcomeSurvey(document) || resolvedAge() != null)) prepareStatusAudit().catch(() => {});
        if (SURVEY_CONFIG.surveyAuditEnabled && requiredNotificationVisible()) revealChecklistAfterOnsNotification();
